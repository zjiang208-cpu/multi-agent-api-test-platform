...
def test_configured_auth_provider_can_refresh_after_token_rejection(monkeypatch):
    ...
    first = asyncio.run(provider.resolve(settings, base_url="http://127.0.0.1:8081", cases=[_case()]))
    cached = asyncio.run(provider.resolve(settings, base_url="http://127.0.0.1:8081", cases=[_case()]))
    provider.invalidate("http://127.0.0.1:8081")
    refreshed = asyncio.run(provider.resolve(settings, base_url="http://127.0.0.1:8081", cases=[_case()]))
